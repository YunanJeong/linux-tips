...
- `777(rwxrwxrwx)`: 완전 허용
  - => 보안정보 유출 및 삭제 위험성
- `644(rw-r--r--)`: 파일 소유자는 읽기/쓰기 가능, 다른 유저는 읽기만 가능.
  - => 보안정보 유출 위험성
- `600(rw-------)`: 파일 소유자만 읽기/쓰기 가능, 다른 유저는 접근 불가.
  - => 가장 안전
- **개별 유저가 root 권한(sudo)를 쓰면 사실상 무의미**하다.
- 다수가 접근하는 서버의 경우 유저 별 root 권한을 제약하는 경우가 있는데, 이 때만 유의미함. 그래도 777은 가급적 피하자.
